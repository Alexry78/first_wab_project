...
ALTER TABLE customers ADD COLUMN balance numeric(12,2);
UPDATE customers SET balance = balance_cents::numeric / 100;
ALTER TABLE customers DROP COLUMN balance_cents;
ALTER TABLE customers ALTER COLUMN balance SET NOT NULL;
ALTER TABLE customers ADD CONSTRAINT balance_check CHECK (balance >= 0);

ALTER TABLE customers ADD COLUMN account_no text;
UPDATE customers SET account_no = 
    'ACC-' || EXTRACT(YEAR FROM registered_at)::text || '-' || LPAD(id::text, 5, '0');
ALTER TABLE customers ALTER COLUMN account_no SET NOT NULL;
ALTER TABLE customers ADD CONSTRAINT account_no_unique UNIQUE (account_no);
...
ALTER TABLE customers ADD CONSTRAINT phone_format_check 
    CHECK (phone ~ '^[0-9]+$' AND LENGTH(phone) BETWEEN 10 AND 15);

UPDATE customers SET status = 'pending' WHERE status IS NULL;
ALTER TABLE customers ADD CONSTRAINT status_check 
